...
from fst.models.model_helper import ModelBuilder
from fst.dataset.builder import get_loader
from fst.dataset.augmentation import generate_unsup_data
from fst.utils.loss_helper import get_criterion
from fst.utils.loss_helper import compute_unsupervised_loss
from fst.utils.loss_helper import compute_contra_memobank_loss
from fst.utils.lr_helper import get_scheduler, get_optimizer
from fst.utils.dist_helper import setup_distributed
from fst.utils.utils import (
    AverageMeter,
...
def main():
    ...
    tmp_model = ModelBuilder(cfg["net"])
    tmp_model = tmp_model.cuda()
    tmp_model = torch.nn.parallel.DistributedDataParallel(
        tmp_model,
        device_ids=[local_rank],
        output_device=local_rank,
        find_unused_parameters=False,
    ...
    # build prototype
    prototype = torch.zeros((cfg["net"]["num_classes"], 256, 1, 256)).cuda()
...
def train(
    ...
    @torch.no_grad()
    def update_ema():
        ema_decay = min(1 - 1 / (i_iter - len(loader_l) * cfg["trainer"].get("sup_only_epoch", 0) + 1),
                        ema_decay_origin)
    ...
    def loss_backward():
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            if np.random.uniform(0, 1) < 0.5 and cfg["trainer"]["unsupervised"].get("apply_aug", False):
                image_u_aug, label_u_aug, logits_u_aug = \
                    generate_unsup_data(image_u, label_u_aug.clone(), logits_u_aug.clone(),
            ...
            with torch.no_grad():
                ...
                pred_u_large_teacher = F.interpolate(pred_u_teacher,
                                                     size=(h, w),
                                                     mode="bilinear",
                                                     align_corners=True)
            ...
            unsup_loss = compute_unsupervised_loss(
                pred_u_large, label_u_aug.clone(),
            ...
            if cfg["trainer"].get("contrastive", False):
                ...
                with torch.no_grad():
                    ...
                    # down sample to [20, 1, 81, 81]

                    if cfg_contra.get("negative_high_entropy", True):
                        contra_flag += " high"
                        high_mask_all = torch.cat(
                            ((label_l.unsqueeze(1) != 255).float(), high_entropy_mask.unsqueeze(1)))
                    else:
                        contra_flag += " low"
                        high_mask_all = torch.cat((
                            (label_l.unsqueeze(1) != 255).float(),
                            torch.ones(logits_u_aug.shape).float().unsqueeze(1).cuda()),
                        )
                    high_mask_all = F.interpolate(high_mask_all, size=pred_all.shape[2:], mode='nearest')  # down sample

                    # down sample and concat
                    label_l_small = F.interpolate(label_onehot(label_l, cfg["net"]["num_classes"]),
                                            size=pred_all.shape[2:], mode='nearest')
                    label_u_small = F.interpolate(label_onehot(label_u_aug, cfg["net"]["num_classes"]),
                                            size=pred_all.shape[2:], mode='nearest')

                prototype, new_keys, contra_loss = compute_contra_memobank_loss(
                    rep_all, label_l_small.long(), label_u_small.long(),
                    prob_l_teacher.detach(), prob_u_teacher.detach(),
                    low_mask_all, high_mask_all,
                    cfg_contra, memobank, queue_ptrlis, queue_size,
                    rep_all_teacher.detach(), prototype,
                    i_iter - len(loader_l) * cfg["trainer"].get("sup_only_epoch", 0) + 1,
                )
        ...
        return sup_loss, unsup_loss, contra_loss, contra_flag
    ...
    for step in range(len(loader_l)):
        ...
        if cfg["trainer"].get("fst", False):
            if epoch < cfg["trainer"].get("sup_only_epoch", 0):
                optimizer.zero_grad()
                sup_loss, unsup_loss, contra_loss, contra_flag = loss_backward()
                optimizer.step()
                update_ema()
