...
def get_rcs_class_probs(data_root, temperature):
    ...
    sample_class_stats = mmcv.load(osp.join(data_root, 'sample_class_stats.json'))
    overall_class_stats = {}
...
class StepUDADataset(object):
    ...
    def __init__(self, source, target, cfg):
        ...
        if self.rcs_enabled:
            ...
            samples_with_class_and_n = mmcv.load(osp.join(cfg['source']['data_root'],
    ...
    def __getitem__(self, idx):
        if self.rcs_enabled:
            data = self.get_rare_class_sample()
            # future batch
            for i in range(self.steps):
                data.update(add_prefix(self.get_rare_class_sample(), 'future_' + str(i)))
